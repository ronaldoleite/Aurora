;(function($) {
  $.fn.quicksearch = function(options) {
    ...
    function make_form_input() {
      ...
      return '<div class="box-8 flex justify-start item-centro"><div> <input type="text" placeholder="Digite um nome, codigo ou data..." value="' + val + '" rel="' + options.randomElement + '" class="' + options.inputClass + '" id="' + options.randomElement + '" /> </div> <div class="psq fonte14 txt-c pd-l-1 pd-r-1 borda-1 hg-40 pd-t-1">Pesquisar</div></div>';
    }

    function make_form_loader() {
      if (!is_empty(options.loaderImg)) {
        return '<img src="' + options.loaderImg + '" alt="" id="' + options.loaderId + '" class="esconde-elemento" />';
      } else {
